...
# 
# name: cleanupserversetup
#
# desc: CAUTION:
#       The script cleanupserversetup.pl can be used to completely clenaup and remove 
#       a IBM Spectrum Protect server that was configured using the blueprint configuration script.
...
#       by the script TSMserverconfig.pl.  The state file is only generated when 
#       cleanupserversetup.pl exists in the same directory as TSMserverconfig.pl when 
#       it is run.
#
#       To use this script:
...
#          prior to running TSMserverconfig.pl
#       3) perl TSMserverconfig.pl
#       4) perl cleanupserversetup.pl
#
# usage:  perl cleanupserversetup.pl
#
...
print "\n*-----------------------------------------------------------------------*\n";
print "** Beginning IBM Spectrum Protect blueprint cleanup script.\n";
print "** $versionString\n";
...
{
  print "\n!! WARNING: This script will cleanup the IBM Spectrum Protect server and all stored data\n";
...
if ((exists $stateHash{db2user}) && ((userexists($stateHash{db2user})) == 1))
{
  ...
  {
    print "Stopping the IBM Spectrum Protect server. Please be patient . . .\n";
  ...
  # Cleanup the IBM Spectrum Protect service
...
if ($instanceexists == 0)
{
  ...
  # cleanup the contents of the IBM Spectrum Protect DB directories
  ...
  # cleanup the contents of the IBM Spectrum Protect DB backup directories
  ...
  # cleanup the contents of the IBM Spectrum Protect storage directories

  print "Cleaning up the IBM Spectrum Protect storage directories\n";
