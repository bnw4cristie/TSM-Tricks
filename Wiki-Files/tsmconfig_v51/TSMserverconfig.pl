...
#
# name: TSMserverconfig.pl
#
# desc: This program is a tool that is intended to facilitate the process of
#       configuring a TSM server on AIX, Linux x86, Linux on Power, or Windows.
...
#       schedules;  At the end of a successful run of this tool, the TSM server is started
...
#       of the document "TSM Blueprint and Server Automated Configuration for Linux x86",
#       "TSM Blueprint and Server Automated Configuration for AIX",
#       or the document "TSM Blueprint and Server Automated Configuration for Windows",
#       as appropriate for your platform.
#
# usage: perl TSMserverconfig.pl [response file path] [-ignoresystemreqs] [-compression]
#
...
if ( $platform eq "WIN32" ) {
    @stepDescArray = (
        ...
        "Formatting the IBM Spectrum Protect server database",
        "Server name and password",
        "IBM Spectrum Protect system administrator name and password",
        "Schedule start time",
    ...
    @stepDescArray = (
        ...
        "Schedule start time",
        "IBM Spectrum Protect server configuration",
        "Setting up the IBM Spectrum Protect server service",
        "Configuration complete"
    );
}
...
# Default paths for the various IBM Spectrum Protect server scales (xsmall, small, medium, large)
...
@promptStringArrayJustContinue =
  ("Press enter to exit the IBM Spectrum Protect server configuration script");
...
@WelcomeStringArray =
  ("Welcome to the IBM Spectrum Protect server configuration tool");
@CongratulationsStringArray1 = ("Congratulations!");
@CongratulationsStringArray2 =
  ("Your IBM Spectrum Protect server is started and ready for use.");

@serverScaleStringArray =
  ("Select the size of your IBM Spectrum Protect server:");
@serverScaleChoiceStringArray =
  ( "X for extra small", "S for small", "M for medium", "L for large" );

$db2instdirString2 =
  "Or, enter a directory for the IBM Spectrum Protect instance: ";
...
$updateDsmservOptString =
  "Updating the IBM Spectrum Protect server option file";
...
$createWinServiceString =
  "Creating the Windows service to run IBM Spectrum Protect";

$adjustLockListString = "Adjusting the DB2 locklist parameter";

$setreorgattribString =
  "Setting IBM Spectrum Protect global attribute for reorg options";
...
$setupServertoStartAtRebootString =
  "Setting up the IBM Spectrum Protect server to start at reboot";

$startingServerString = "Starting the IBM Spectrum Protect server";

$sysadminidString1 =
"Press enter to accept the default IBM Spectrum Protect system administrator ID [admin].";
$sysadminidString2 =
  "Or, enter a IBM Spectrum Protect system administrator ID: ";

$sysadminpwString1 =
"A password is required for the default IBM Spectrum Protect system administrator.";
$sysadminpwString2 =
  "Enter a IBM Spectrum Protect system administrator password that is a minimum of $minPWlength characters: ";
...
# Populate array containing valid characters for IBM Spectrum Protect Server objects
...
#     desc: Prompts for, and obtains the IBM Spectrum Protect server instance directory
...
sub getInstanceDirectory {
    ...
    logentry("Step ${stpn}: Get the IBM Spectrum Protect instance directory\n");
...
#     desc: Prepares the IBM Spectrum Protect server reorg related options
#           depending on which IBM Spectrum Protect server level is being
...
#           of the IBM Spectrum Protect system administrator
#
#   params: 1. the step number
#  returns: none
#
...
sub getsysadminCredentials {
    ...
    logentry(
"Step ${stpn}: Get the IBM Spectrum Protect system administrator credentials\n"
    );
    ...
    do {
        ...
        if ( $rc == 0 ) {
            if ( $foundinputinfile == 1 ) {
                displayString( 10, 3,
                    "IBM Spectrum Protect system administrator ID> $sysadminid"
                );
                sleep 2;
            }
        }
    ...
    if ( $doneflag == 0 ) {
        ...
        logentry(
            "        User response: IBM Spectrum Protect system administrator id: $sysadminid\n"
        );
        ...
        do {
            ...
            if ( $foundinputinfile == 1 ) {
                displayString( 10, 3, "IBM Spectrum Protect system administrator password> ********");
                sleep 2;
            }
        ...
        if ( $doneflag == 0 ) {
            ...
            logentry(
"        User response: IBM Spectrum Protect system administrator password ********\n"
            );
...
#           for IBM Spectrum Protect storage from the user. These directories are then
...
sub getTsmStoragePaths {
    ...
    logentry("Step ${stpn}: Get the IBM Spectrum Protect storage paths\n");
...
"Or, provide at least $expected_number_of_dirs filesystems for the storage directories";
        ...
        if ( ( $doneflag == 0 ) && ( $repeatstep == 0 ) ) {
            ...
            logentry(
"        User response: IBM Spectrum Protect storage directories: $userresponseList\n"
            );
            ...
            if ( $numberofpaths >= $expected_number_of_dirs ) {
                ...
                {
                    logentry(
"        Total free space in the IBM Spectrum Protect storage directories is $stgdirstotalfreespace, expected is $stateHash{stgdirsfreespacemin}\n"
                    );
                    logentry(
"        The amount of free space for the IBM Spectrum Protect storage directories is not sufficient for $stateHash{serverscale} server\n"
                    );
                    ...
                    $enclmntpnt = getencompassingmountpoint($instdirmountpoint);
                    logentry(
"        The free space required for the IBM Spectrum Protect storage directories is $stateHash{stgdirsfreespacemin}"
                    );
                    logentry(
"        Total free space in the IBM Spectrum Protect storage directories is $stgdirstotalfreespace\n"
                    );
...
############################################################
#      sub: chownPaths
#     desc: chowns the db instance directory, the db directories, the IBM Spectrum Protect storage
...
#     desc: prepares the IBM Spectrum Protect API client option files that will
#           be needed for backing up the server database. Also, for
#           server version below 7, the password file (TSM.PWD) is
#           generated using the dsmputil utility. The IBM Spectrum Protect BA client
#           option files are prepared by this subroutine, for later use
#           when connecting to the IBM Spectrum Protect server by way of the admin
#           client
#
#   params: 1. the step number
#
...
# Beginning with IBM Spectrum Protect 812, we no longer need to use dsmsutil.exe to store a DB backup password
...
sub setreorgflag {
    ...
    logentry(
"Step ${stpn}_${substep}: Prepare command file to set IBM Spectrum Protect reorg attribute\n",
        1
    );
...
"insert into TSMDB1.global_attributes (owner,name,type,length,int32) values('RDB','REORG_ONETIME_AUTO_INIT',3,0,2)";
        ...
        logentry(
"        An error occurred when attempting to create the sql file to set the IBM Spectrum Protect reorg attrib\n"
        );
        return 1;
    }
    ...
    if ( open( SETDB2LLCMDH, ">$setdb2attribcmdfile" ) ) {
        ...
        logentry(
"       An error occurred when attempting to create the command file to set the IBM Spectrum Protect reorg attrib\n"
        );
        return 1;
    }

    logentry(
"Step ${stpn}_${substep}: Set the IBM Spectrum Protect flag to avoid automatic setting of reorg options\n",
        1
    );
    ...
    if ( $setdb2attribrc == 0 ) {
        ...
        logentry(
"        An error occurred when attempting to set the IBM Spectrum Protect global reorg attrib\n"
        );
        my @rcArray =
          ( 1, "error setting the IBM Spectrum Protect global reorg attrib" );
        return @rcArray;
    }
}
...
#     desc: Creates the Windows service to run the IBM Spectrum Protect server
#
#   params: 1. the step number
#
#  returns: an array consisting of a return code (0 for success,
...
sub createWinService {
    ...
    logentry(
"Step ${stpn}_${substep}: Create the Windows service to run the IBM Spectrum Protect server\n",
        1
    );
...
"sc description \"TSM server_${db2usr}\" \"IBM Spectrum Protect Server\"";
        `$descriptionTSMserviceCmd`;
        ...
        logentry(
"        An error occurred when attempting to create the Windows service to run the IBM Spectrum Protect server\n"
        );
...
sub formatserver {
    ...
    logentry(
        "Step ${stpn}_${substep}: Format the IBM Spectrum Protect server\n",
        1 );
...
"        While the successful offline backup message was not found, the file $dbidfile exists\n"
            ...
            logentry(
"        For now, this will be considered sufficient evidence that the IBM Spectrum Protect server format was successful\n"
            );
            $formatserverrc = 0;
        }
    }
...
"        Adjusting DB2 settings for locklist, system account access, and reorg attribute\n"
        ...
        logentry(
"        An error occurred when attempting to format the IBM Spectrum Protect server\n"
        );
...
sub initializeServer {
    ...
    if ( ( $genrunfilerc == 0 ) && ( $gentsmconfigmacrorc == 0 ) ) {
        ...
        logentry(
"Step ${stpn}_${substep}: Initialize the IBM Spectrum Protect server\n",
            1
        );
        ...
        if ( $startserverrc == 0 ) {
            ...
            logentry(
"        An error occurred when attempting to initialize IBM Spectrum Protect server\n"
            );
            $okstring = genresultString( $generateRunfileString, 55, "[OK]" );
            displayString( 10, 3, $okstring, 1, $stpn );
...
"Step ${stpn}_${substep}: Update the server option file with the appropriate activelogsize\n",
        ...
        if ( $updateactlogsize == 1 ) {
            logentry(
"Step ${stpn}_${substep}: Start up the IBM Spectrum Protect server\n",
                1
            );
...
"${serverPath}${SS}rc.dsmserv -u $db2usr -i $instdirmntpnt -q &";
        ...
        if ( $serverStarted == 1 ) {
            logentry(
                "        The IBM Spectrum Protect server started successfully\n"
            );
            $okstring1 = genresultString( $generateRunfileString, 55, "[OK]" );
            displayString( 10, 3, $okstring1, 1, $stpn );
            $okstring2 = genresultString( $initServerString1, 55, "[OK]" );
            ...
            logentry(
"        An error occurred when attempting to start the IBM Spectrum Protect server\n"
            );
            $okstring1 = genresultString( $generateRunfileString, 55, "[OK]" );
            displayString( 10, 3, $okstring1, 1, $stpn );
            $okstring2 = genresultString( $initServerString1, 55, "[OK]" );
    ...
    if ( $serverStarted == 1 ) {
        ...
        logentry(
"Step ${stpn}_${substep}: Connect to IBM Spectrum Protect server and complete the server configuration\n",
            1
        );
...
"error trying to prepare batch file to connect to the server"
        ...
        if ( $serverconfigcomplete == 1 ) {
            logentry(
"        IBM Spectrum Protect server configuration completed successfully\n"
            );
            $okstring1 = genresultString( $generateRunfileString, 55, "[OK]" );
            displayString( 10, 3, $okstring1, 1, $stpn );
            $okstring2 = genresultString( $initServerString1, 55, "[OK]" );
...
"        An error occurred when attempting to create pre-allocated volumes\n"
            ...
            logentry(
"        An error occurred when attempting to configure the IBM Spectrum Protect server\n"
            );
            $okstring1 = genresultString( $generateRunfileString, 55, "[OK]" );
            displayString( 10, 3, $okstring1, 1, $stpn );
            $okstring2 = genresultString( $initServerString1, 55, "[OK]" );
...
#           location, and registers the IBM Spectrum Protect server as a service
...
sub setupforstartatreboot {
    ...
    logentry(
"Step ${stpn}: Set up IBM Spectrum Protect server to start up at reboot\n"
    );
...
# may yield strange output
    ...
    if ( $platform ne "WIN32" ) {
        if ( validateHostsFile() == 0 ) {
            die
"\nPlease verify there is an entry for the hostname in the /etc/hosts file, in accordance with IBM Spectrum Protect Blueprint instructions\n\n";
        }
    }
...
# Beginning with IBM Spectrum Protect 812, we need to password protect the database backups.
    ...
    # Beginning with IBM Spectrum Protect 713, we will build a container stgpool
...
sub getlogHeader {
    ...
    $line =
"********************************************************************************\n";
    my $longtestcase = "**  IBM Spectrum Protect server setup script log\n";
    my $date =
...
#           IBM Spectrum Protect server as a service
#
#   params: 1. the step number
#
#  returns: 0 if successful
...
"        An error occurred when attempting to read the dsmserv.rc file\n"
    ...
    if ( $issystemctl == 1 ) {
        if ( open( FH, '>', $systemdfile ) ) {
            print FH "[Unit]\n";
            print FH
              "Description=IBM Spectrum Protect Server instance ${db2user}\n";
            print FH "\n";
            print FH "[Service]\n";
...
############################################################
#      sub: configservice
#     desc: Invokes chkconfig to configure the IBM Spectrum Protect server to
#           start at boot
#
#   params: 1. the step number
#
...
#     desc: Invokes systemd to configure the IBM Spectrum Protect server to
#           start at boot
#
#   params: 1. the step number
#
...
#           IBM Spectrum Protect server is being formatted
#
...
"reg query HKLM${SS}SOFTWARE${SS}IBM${SS}ADSM${SS}CurrentVersion${SS}AdminClient";
    ...
    if ( $serverinstalled == 0 ) {
        ...
        logentry(
"Exiting due to IBM Spectrum Protect server installation not detected\n"
        );
        die
"This configuration script requires that the IBM Spectrum Protect server be installed\n";
    }

    if ( $serverlicenseinstalled == 0 ) {
        logentry("Unexpected result from: $queryserverlicensekeycmd\n");
        logentry(
"Exiting due to IBM Spectrum Protect server license installation not detected\n"
        );

        die
"This configuration script requires that the IBM Spectrum Protect server license be installed\n";
    }

    if ( $apiclientinstalled == 0 ) {
        logentry("Unexpected result from: $queryapiclientkeycmd\n");
        logentry(
"Exiting due to IBM Spectrum Protect API client installation not detected\n"
        );

        die
"This configuration script requires that the IBM Spectrum Protect API client be installed\n";
    }

    if ( $baclientinstalled == 0 ) {
        logentry("Unexpected result from: $querybaclientkeycmd\n");
        logentry(
"Exiting due to IBM Spectrum Protect BA client installation not detected\n"
        );

        die
"This configuration script requires that the IBM Spectrum Protect BA client be installed\n";
    }

    if ( ( $platform eq "WIN32" ) && ( $adminclientinstalled == 0 ) ) {
        logentry("Unexpected result from: $queryadminclientkeycmd\n");
        logentry(
"Exiting due to IBM Spectrum Protect BA administrative client installation not detected\n"
        );

        die
"This configuration script requires that the IBM Spectrum Protect administrative client be installed\n";
    }
...
"The -compression flag should only be used with V7 servers and newer\n";
    ...
    if ( $platform eq "WIN32" ) {
        logentry(
"Successfully verified prerequisites: IBM Spectrum Protect server, server license, API client, BA client and administrative client\n"
        );
    }
    else {
        logentry(
"Successfully verified prerequisites: IBM Spectrum Protect server, server license, API client and BA client\n"
        );
    }
...
sub validatePassword {
    ...
    # Make sure the password only contains characters acceptable to Spectrum Protect
    my $tempstr        = $pswd;
    my $validcharfound = 0;
