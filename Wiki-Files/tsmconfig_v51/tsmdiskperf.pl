...
# name: tsmdiskperf
#
# desc: Simulates typical IBM Spectrum Protect workload patterns for assessing
...
#
# usage:  perl tsmdiskperf.pl workload=stgpool|db [mode=readonly|writeonly|writeread|writerandread|randreadonly] [readers=num] 
...
else  # build a path based on the full path to tsmdiskperf.pl
{
  $fullPath = $0;
  $fullPath =~ m/(.*)(\/|\\)(\w|\.)+$/;
  $ddName = $1.$2.$ddName;
...
if (@ARGV < 1 || @ARGV > 5)
{
  print "USAGE: perl tsmdiskperf.pl workload=db|stgpool [fslist=/fs1,/fs2,...]\n";
...
# Variables
$filePrefix = "tsmdiskperf";       # prefix of filenames like tsmdiskperf0_1
...
print "======================================================================\n";
print ": IBM Spectrum Protect disk performance test\t($versionString)\n";
print ":\n";
...
if ($workload eq "db" || ($workload eq "stgpool" && ($mode eq "readonly" || $mode eq "randreadonly")))
{
  ...
  if ($workload eq "db")
  {
    print ": Creating files of $fileSizeGB GB to simulate IBM Spectrum Protect DB.\n";
  }
  else
  {
    print ": Creating files of $fileSizeGB GB to prepare to simulate IBM Spectrum Protect Stgpool for reads.\n";
  }
  my @child_pids=();
  my $ddcmd = "";
  foreach $fs (@fsArray)
